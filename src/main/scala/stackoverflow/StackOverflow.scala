...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(Int, Iterable[(Posting, Posting)])] = {
    val questions =
      postings.filter(_.postingType == 1)
              .map(question => (question.id, question))
    val answers =
      postings.filter(answer => answer.postingType == 2 && answer.parentId.isDefined)
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val medianScore: Int    = {
        ...
        val sortedScore = vs.map(_._2).toList.sorted
        if (clusterSize % 2 == 0) (sortedScore(middle) + sortedScore(middle - 1)) / 2
        else sortedScore(middle)
      }
